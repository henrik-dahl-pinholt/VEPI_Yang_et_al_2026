...
cross_corr_lags = np.arange(-50, 100 + 1)
frame_dt_min = time_interval_seconds(time_interval) / 60.0
cross_corr_lags_min = cross_corr_lags * frame_dt_min
lags_pon = np.arange(0, 400)
agg_acfs = []
agg_pol2_cross_correlation = []
agg_pol2_turnon_cross_correlation = []
agg_on_on_auto = []
agg_off_off_auto = []
agg_turnon_auto = []
agg_lrates = []
agg_pol2_cross_correlation_predict = []
# acf_lags = np.arange(100 + 1)
for fit_type in ["Regular", "Shifted", "Shuffled"]:
    acfs = []
    pol2_cross_correlation = []
    pol2_turnon_cross_correlation = []
    on_on_auto = []
    off_off_auto = []
    lrates = []
    pol2_cross_correlation_predict = []
    turnon_auto = []
    for condition in conditions:
        ...
        ep_data = model_res["ep_data"]
        if model_res is None:
            print(f"Model result is None: {model_result_path}")
            continue
        ...
        paramvec = msd_fit["compact_gpr_params"]
        p2_load = model_res["pol2_loading_posterior"]
        pcont = model_res["pcont"]
        pon = model_res["pon"]
        valid_ms2 = ~np.isnan(model_res["ms2"])
        p2_load = np.where(valid_ms2, p2_load, np.nan)
        pcont = np.where(valid_ms2, pcont, np.nan)
        pon = np.where(valid_ms2, pon, np.nan)
        if fit_type == "Regular":
            contact_autocorr = np.asarray(
                cross_corr_jax(pcont, pcont, cross_corr_lags),
                dtype=float,
            )
            pon_autocorr = np.asarray(
                cross_corr_jax(pon, pon, cross_corr_lags),
                dtype=float,
            )
            avg_load = np.nanmean(model_res["loading_rates"][:, 0])
            pred = predict_w0_pon_crosscorr(
                cross_corr_lags_min,
                contact_autocorr,
                pon_autocorr,
                frame_dt_min,
                max_delay_min=200.0,
                response_lag_min=-1.0 / avg_load,
            )
            pol2_cross_correlation_predict.append(pred)

        cross_corr = cross_corr_jax(pcont, p2_load, cross_corr_lags)
        pol2_cross_correlation.append(cross_corr)
        pol2_turnon_cross_correlation.append(
            cross_corr_jax(model_res["turn_on"], p2_load, cross_corr_lags)
        )

        on_on_auto.append(acf_jax(pon, lags_pon))
        off_off_auto.append(acf_jax(1 - pon, lags_pon))
        turnon_auto.append(acf_jax(model_res["turn_on"], lags_pon))
        lrates.append(model_res["loading_rates"])
    agg_acfs.append(acfs)
    agg_pol2_cross_correlation_predict.append(pol2_cross_correlation_predict)
    agg_pol2_cross_correlation.append(pol2_cross_correlation)
    agg_pol2_turnon_cross_correlation.append(pol2_turnon_cross_correlation)
    agg_on_on_auto.append(on_on_auto)
    agg_off_off_auto.append(off_off_auto)
    agg_turnon_auto.append(turnon_auto)
    agg_lrates.append(lrates)

avg_lrates = [np.mean(np.concatenate(lrates)[:, 0], axis=0) for lrates in agg_lrates]

dt = float(time_interval.split("s")[0])
arr_auto = np.array(agg_turnon_auto[0])
arr_auto = arr_auto / arr_auto[:, 0][:, None]
fig, ax = plt.subplots(1, 1, figsize=(5, 5))
ax.plot(lags_pon * dt, arr_auto.T, "o")
tx = ax.twinx()
tx.plot(lags_pon * dt, np.cumsum(arr_auto, axis=1).T * dt, label=conditions)
tx.legend()
ax.set(xlim=(0, 700))


fig, ax = plt.subplots(1, 1, figsize=(5, 5))
for m, pol2_cross_correlation in enumerate(agg_pol2_cross_correlation):
    # dt_fine = fine_times[1] - fine_times[0]
    if m == 0:
        for cross_corr, label in zip(pol2_cross_correlation, conditions):
            if label == conditions[0]:
                ax.plot(
                    cross_corr_lags_min,
                    cross_corr,
                    alpha=0.2,
    ...
    # avg_curve = smooth_lag_curve(np.mean(pol2_cross_correlation, axis=0))
    avg_curve = np.mean(pol2_cross_correlation, axis=0)
    ax.plot(
        cross_corr_lags_min,
        avg_curve,
        label=f"Average {['Regular','Shifted','Shuffled'][m]}",
        color=["C0", "C1", "C2"][m],
        linewidth=3,
    )
    amax = np.argmax(avg_curve)
    maxval = avg_curve[amax]
    # ax.axvline(x=cross_corr_lags[amax] * 0.5, color="red", linestyle="--")
avg_pred = np.nanmean(np.asarray(agg_pol2_cross_correlation_predict[0]), axis=0)
avg_regular = np.nanmean(np.asarray(agg_pol2_cross_correlation[0]), axis=0)
prediction_fit_mask = (cross_corr_lags_min >= -5.0) & (cross_corr_lags_min <= 15.0)
pred_scale = fit_amplitude_only(avg_regular, avg_pred, prediction_fit_mask)
avg_pred = pred_scale * avg_pred

ax.plot(
    cross_corr_lags_min,
    avg_pred,
    label=f"Instantaneous activation prediction",
    color="k",
    linewidth=3,
    zorder=-1,
)
...
ax.set(
    xlabel="Lag (minutes)",
    ylabel="<contact_t pol2loading_{t+lag}>",
    # ylim=(-0.015, 0.035),
)
ax.legend()
fig.tight_layout()
fig.savefig(MAIN_FIG_DIR / "contact_pol2_cross_corr.pdf")
